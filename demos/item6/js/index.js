	...
	render:function(){
		...
		if(this.props.todoItem.isEdit){
			ele=<input className="editItem"  defaultValue={this.state.editTitle}  onKeyUp={this.itemEditCompleted} onBlur={this.blurEditCompleted} autofocus />
				...
				ele=<p onDoubleClick={this.itemEdit} style={styleObj}><span className="text">{this.props.todoItem.title}</span><button ref="deleButton"   className="del" onClick={this.handlerDelItem} >删除</button></p>
...
app.App=React.createClass({
	...
	delAllCompleted:function(){
		...
		var remain=this.state.todos.filter((item)=>{
			return item.isCompleted === false;
		})
