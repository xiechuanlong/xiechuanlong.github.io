...
/***/ function(module, exports, __webpack_require__) {
	...
	var _jsIndexJs = __webpack_require__(1);

	var _jsIndexJs2 = _interopRequireDefault(_jsIndexJs);
...
/***/ function(module, exports, __webpack_require__) {
	...
	app.Header = _react2['default'].createClass({
		displayName: 'Header',
		...
		handlerEdit: function handlerEdit(event) {
			if (event.keyCode === 13) {
				var title = event.target.value;
				if (!title) {
					return false;
		...
		render: function render() {
			return _react2['default'].createElement('input', { className: 'add-new-todo', type: 'text', placeholder: 'what need to do ?', onKeyUp: this.handlerKeyUp, autofocus: true });
		}
	});
	...
	var Item = _react2['default'].createClass({
		...
		componentWillReceiveProps: function componentWillReceiveProps(newProps) {
			this.state.editTitle = newProps.todoItem.title;
			this.setState(this.state);
		},
		//删除一个todo的事件函数
		handlerDelItem: function handlerDelItem(event) {
			this.props.delTodoItem(this.props.index);
		},

		//鼠标移入的数据函数
		handlerMouseOver: function handlerMouseOver() {
			if (this.refs.deleButton) {
				_reactDom2['default'].findDOMNode(this.refs.deleButton).style.display = "inline-block";
			}
		},

		//鼠标移除时的事件函数
		handlerMouseOut: function handlerMouseOut() {
			if (this.refs.deleButton) {
				_reactDom2['default'].findDOMNode(this.refs.deleButton).style.display = "none";
			}
		},

		//点击切换完成与未完成状态
		handlerToggle: function handlerToggle() {
			var isCompleted = !this.props.todoItem.isCompleted;
		...
		//编辑完成
		itemEditCompleted: function itemEditCompleted(event) {
			var title = event.target.value;
		...
		blurEditCompleted: function blurEditCompleted(event) {
			this.itemEditCompleted(event);
		},

		render: function render() {
			var ele = undefined;
			var styleObj = this.props.todoItem.isCompleted ? { color: "gray", textDecoration: "line-through" } : {};
			if (this.props.todoItem.isEdit) {
				ele = _react2['default'].createElement('input', { className: 'editItem', defaultValue: this.state.editTitle, onKeyUp: this.itemEditCompleted, onBlur: this.blurEditCompleted, autofocus: true });
			} else {

				ele = _react2['default'].createElement(
					'p',
					{ onDoubleClick: this.itemEdit, style: styleObj },
					...
					_react2['default'].createElement(
						'button',
						{ ref: 'deleButton', className: 'del', onClick: this.handlerDelItem },
						'删除'
					)
				);
			}
			return _react2['default'].createElement(
				...
				_react2['default'].createElement('input', { className: 'item-toggle', type: 'checkbox', checked: this.props.todoItem.isCompleted, onChange: this.handlerToggle }),
				ele
			);
		}
	});

	app.Main = _react2['default'].createClass({
		displayName: 'Main',
			...
			return _react2['default'].createElement(
				...
				_react2['default'].createElement(
					...
					this.props.todos.map(function (item, index) {
						return _react2['default'].createElement(Item, { delTodoItem: _this.props.delTodoItem, todoItem: item, key: index, index: index, itemCompleteToggle: _this.props.itemCompleteToggle, itemEdit: _this.props.itemEdit, itemEditCompleted: _this.props.itemEditCompleted });
					})
				)
			);
		}
	...
	//Footer部分组件
	app.Footer = _react2['default'].createClass({
		displayName: 'Footer',

		handlerToggleAll: function handlerToggleAll() {
			this.props.itemsToggleAll(this.props.isAllCompleted);
		},
		handlerDelAllCompleted: function handlerDelAllCompleted() {
		...
		render: function render() {
			return _react2['default'].createElement(
				...
				_react2['default'].createElement(
					'p',
					null,
					_react2['default'].createElement('input', { type: 'checkbox', checked: this.props.isAllCompleted, onChange: this.handlerToggleAll }),
					_react2['default'].createElement(
						'span',
						null,
						this.props.isCompletedCount,
						'个已经完成/',
						this.props.totleCount,
						' 总数'
					),
					_react2['default'].createElement(
						'button',
						{ className: 'clearAll', onClick: this.handlerDelAllCompleted },
						'删除已完成'
					)
				)
			);
	...
	//整个Todo-App组件
	app.App = _react2['default'].createClass({
		displayName: 'App',
		...
		//添加一个todo时的数据变化
		addTodo: function addTodo(newTodoTitle) {
			var o = {};
		...
		//删除一个todo时的数据变化
		delTodoItem: function delTodoItem(index) {
			this.state.todos.splice(index, 1);
			this.setState({ todos: this.state.todos });
		},

		//toggle一个item时完成状态数据变化
		itemCompleteToggle: function itemCompleteToggle(index, isCompleted) {
		...
		//双击item编辑事件
		itemEdit: function itemEdit(index) {
			this.state.todos[index].isEdit = true;
			this.setState({ todos: this.state.todos });
		},

		//编辑完成
		itemEditCompleted: function itemEditCompleted(index, value) {
		...
		itemsToggleAll: function itemsToggleAll(isAllCompleted) {
			var itemsState = this.state.todos.map(function (item) {
		...
		//删除所有已完成item的事件
		delAllCompleted: function delAllCompleted() {
			// this.setState({todos:this.state.todos.filter((item)=>{
			// 	return item.isCompleted === false;
			// })})   此处this.state.todos未发生改变
			var remain = this.state.todos.filter(function (item) {
				return item.isCompleted === false;
			});
			this.state.todos = remain;
		...
		render: function render() {
			var isCompletedCount = this.state.todos && this.state.todos.filter(function (todo) {
				return todo.isCompleted;
			}).length || 0;
			var totleCount = this.state.todos.length || 0;
			...
			return _react2['default'].createElement(
				...
				_react2['default'].createElement(
					'div',
					{ id: 'app' },
					_react2['default'].createElement(app.Header, { addTodo: this.addTodo }),
					_react2['default'].createElement(app.Main, { todos: this.state.todos, delTodoItem: this.delTodoItem, itemCompleteToggle: this.itemCompleteToggle, itemEdit: this.itemEdit, itemEditCompleted: this.itemEditCompleted }),
					_react2['default'].createElement(app.Footer, { delAllCompleted: this.delAllCompleted, itemsToggleAll: this.itemsToggleAll, isCompletedCount: isCompletedCount, totleCount: totleCount, isAllCompleted: this.state.isAllCompleted })
				)
			);
		}
	});
	_reactDom2['default'].render(_react2['default'].createElement(app.App, null), document.getElementById("todo-app"));
